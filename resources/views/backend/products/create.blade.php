...
@section('title', ucwords(trans('title.create', ['attribute' => 'product'])))
    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card-box">
                ...
                <form class="w-100" data-parsley-validate action="{{ route('admin.products.store') }}" novalidate enctype="multipart/form-data" method="POST">
                    @csrf
                    <div class="form-row">
                        <div class="form-group col-md-6">
                            <label class="text-capitalize col-form-label" for="name">{{ trans('form-label.name') }}<span class="text-danger"> (*)</span></label>
                            <input id="name" class="form-control @error('name') parsley-error @enderror" name="name" type="text" value="{{ old('name') }}" parsley-trigger="change" required placeholder="Enter product's name">
                            @error('name')
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-6">
                            ...
                            <input id="price" class="form-control @error('price') parsley-error @enderror" name="price" type="number" min="0" value="{{ old('price') }}" parsley-trigger="change" required placeholder="Enter product's price">
                            @error('price')
                                <ul class="parsley-errors-list filled" aria-hidden="false">
                                    <li class="parsley-required">{{ $message }}</li>
                                </ul>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input id="quantity" class="form-control @error('quantity') parsley-error @enderror" name="quantity" type="number" min="0" value="{{ old('quantity') }}" parsley-trigger="change" required placeholder="Enter product's quantity">
                            @error('quantity')
                                <ul class="parsley-errors-list filled" aria-hidden="false">
                                    <li class="parsley-required">{{ $message }}</li>
                                </ul>
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="description" class="form-control @error('description') parsley-error @enderror" name="description" placeholder="Enter product's description" rows="6">{{ old('description') }}</textarea>
                        @error('description')
                            <ul class="parsley-errors-list filled" aria-hidden="false">
                                <li class="parsley-required">{{ $message }}</li>
                            </ul>
